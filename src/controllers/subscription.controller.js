...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    if (!isSubscribed){
        ...
        return res.status(200).json(new ApiResponse(200, { subscribed: true }, "Sunscribed successfully"));
    }
    ...
    return res.status(200).json(new ApiResponse(200, { subscribed: false }, "Unsunscribed successfully"));
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
    ...
    const subscriptions = await Subscription.find({channel : channelId}).populate('subscriber', 'username email avatar');
...
// ALTERNATE : USING MONGODB PIPELINES :
...
const getSubscribedChannels = asyncHandler(async (req, res) => {
    ...
    if (!subscribedChannels || subscribedChannels){
        throw new ApiError(400, "No channel found for this subscriber")
    }

    const channels = subscribedChannels.map(ch => ch.channel);
